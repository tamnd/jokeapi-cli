// Package cli assembles the jokeapi command tree from the jokeapi
...
// NewApp assembles the kit application from the jokeapi domain. The
...
//
// To add a command, declare it in jokeapi/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := jokeapi.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(jokeapi.Domain{}).Register(app)
