...
describes what users get. Pick a format with `-o`, or let jokeapi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
jokeapi <command> -o table   # aligned columns for reading
jokeapi <command> -o jsonl   # one JSON object per line, for piping
jokeapi <command> -o json    # a single JSON array
jokeapi <command> -o csv     # spreadsheet friendly
jokeapi <command> -o tsv     # tab-separated
jokeapi <command> -o url     # just the URL column
jokeapi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
jokeapi <command> --fields id,title,url
```
...
```bash
jokeapi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
jokeapi <command>            # a table, because this is a terminal
jokeapi <command> | wc -l    # JSONL, because this is a pipe
```
